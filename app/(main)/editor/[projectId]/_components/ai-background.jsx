...
export default function BackgroundControls({ project }) {
  ...
  // New state for AI background text input
  const [aiBackgroundText, setAiBackgroundText] = useState("");
  ...
  const handleAIPrompt = async (prompt) => {
    ...
    setProcessingMessage(null);  // <-- clear loading here
  }
  }
...
return (
  <div className="space-y-6 relative h-full">
    {/* New AI Background Text Input + Submit */}
    <div className="mb-6 flex gap-2">
      <Input
        value={aiBackgroundText}
        onChange={(e) => setAiBackgroundText(e.target.value)}
        onKeyDown={(e) => {
        if (e.key === "Enter") {
          e.preventDefault();
          handleAIPrompt(aiBackgroundText);
        }
      }}
        placeholder="Enter text for AI background"
        className="flex-1 bg-slate-700 border-white/20 text-white"
      />
      <Button
      onClick={() => handleAIPrompt(aiBackgroundText)}
      variant="primary"
    >
      Submit
    </Button>
